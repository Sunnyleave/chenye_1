...
def evaluate(truth_val, n_answered):
    return np.count_nonzero(truth_val - ground_truth_new) / (numS * n_answered * numA)
            ...
            for a in range(numA):
                for s in range(numS):
                    if np.max(data_mat_const[s, e]) > 0:
                        ...
                        truth[s][e][a] = np.argmax(np.bincount(claim_list))
            num_answered += 1
            ...
            for s in range(numS):
                ...
                if np.max(data_mat_const[s, e]) > 0:
                    ...
                    if samples[i] < 0.5:  # Change dependence attribute
                        if truth[s][e][pair[0]] in fact_list:
                            ...
                            if fact_list[truth[s][e][pair[0]]] != truth[s][e][pair[1]]:
                                # num_changed += 1
                                # if fact_list[att_claim_list[s][e][0]] == ground_truth[e][pair[1]]:
                                #     num_change2_true += 1
                                truth[s][e][pair[1]] = fact_list[truth[s][e][pair[0]]]
                                #print(s,e,pair[1])
                                #print(truth[s][e][pair[1]])
                                if att_claim_list[s][e][1] != truth[s][e][pair[1]]:
                                    num_changed += 1
                                    if truth[s][e][pair[1]] == ground_truth_new[s][e][pair[1]]:
                                        num_change2_true += 1
                                if att_claim_list[s][e][0] != truth[s][e][pair[0]]:
                                    num_changed += 1
                                    if truth[s][e][pair[0]] == ground_truth_new[s][e][pair[0]]:
                                        num_change2_true += 1

                        else:
                            fact_list[truth[s][e][pair[0]]] = truth[s][e][pair[1]]
                            if att_claim_list[s][e][1] != truth[s][e][pair[1]]:
                                num_changed += 1
                                if truth[s][e][pair[1]] == ground_truth_new[s][e][pair[1]]:
                                    num_change2_true += 1
                        ...
                        if truth[s][e][pair[0]] in fact_list:
                            if fact_list[truth[s][e][pair[0]]] != truth[s][e][pair[1]]:
                                truth[s][e][pair[0]] = random.choice(whole_set)
                                # print(e)
                                # print(truth[e][pair[0]])
                                if att_claim_list[s][e][0] != truth[s][e][pair[0]]:
                                    num_changed += 1
                                    if truth[s][e][pair[0]] == ground_truth_new[s][e][pair[0]]:
                                        num_change2_true += 1
